...
fn spawn_prewarm(client: ModelClient, model_info: ModelInfo, session_telemetry: SessionTelemetry) {
    tokio::spawn(async move {
        let mut session = client.new_session();
        let prompt = Prompt::default();
        if let Err(e) = session
            .prewarm_websocket(&prompt, &model_info, &session_telemetry, None, ReasoningSummaryConfig::Auto, None)
            .await
        {
...
async fn acquire_slot(
    ...
    if is_new {
        spawn_prewarm(client.clone(), model_info.clone(), state.session_telemetry.clone());
    }
    client
}
...
async fn handle_messages(
    ...
    let stream_result = session
        .stream(&prompt, &model_info, &state.session_telemetry, stream_effort, reasoning_summary, None)
        .await;
